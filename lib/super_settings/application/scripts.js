(function() {
  ...
  function sortOrder() {
    const selectedSort = document.querySelector(".js-sort-order[data-selected=true]");
    const field = selectedSort.closest("[data-field]").dataset.field;
  ...
  function setSortOrder(event) {
    ...
    const target = event.target.closest(".js-sort-order");
    let newSelection = null;
    let prevSelection = document.querySelector(".js-sort-order[data-selected=true]");

    if (prevSelection != target) {
      newSelection = target;
    } else {
      prevSelection = target;
      prevSelection.style.display = "none";
      newSelection = Array.from(target.closest("button").querySelectorAll(".js-sort-order")).find(function(element) {
        return element !== target;
      });
      newSelection.style.display = "inline-block";
    }

    selectSortElement(prevSelection, false);
    selectSortElement(newSelection, true);
  ...
  function selectSortElement(element, selected) {
    ...
    if (selected) {
      svg.style.backgroundColor = getComputedStyle(document.querySelector(".super-settings")).getPropertyValue("--selected-color");
  ...
  docReady(function() {
    ...
    addListener(document.querySelectorAll(".js-sort-order"), "click", setSortOrder);

    const queryParams = new URLSearchParams(window.location.search);
    applyFilter(queryParams.get("filter"));

    selectSortElement(document.querySelector(".js-sort-order"), true);
