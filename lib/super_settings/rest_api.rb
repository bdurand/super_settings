...
module SuperSettings
  class RestAPI
    ...
    #   ]
    def index
      settings = @settings.active.sort_by(&:key)
    ...
    #   }
    def show(key)
      @settings.find_by_key(key)&.as_json
    end
    ...
    def update(settings_params, changed_by = nil)
      all_valid, settings = @settings.bulk_update(Array(settings_params), changed_by)
      if all_valid
        {success: true}
      else
        errors = {}
    ...
    #   }
    def last_updated_at
      {last_updated_at: @settings.last_updated_at.utc.iso8601}
    end
    ...
    def updated_since(time)
      ...
      settings = @settings.updated_since(time)
      {settings: settings.collect(&:as_json)}
    end
  end
end
