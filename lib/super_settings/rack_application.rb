...
module SuperSettings
  ...
  class RackApplication
    ...
    #     end
    #   end
    def initialize(app = nil, path_prefix = "/", namespace = nil, &block)
    ...
    def handle_root_request(request)
      response = check_authorization(request, write_required: true) do |user|
        ...
        application = Application.new(namespace: namespace, layout: :default, add_to_head: add_to_head(request))
    ...
    def handle_index_request(request)
      check_authorization(request) do |user|
        json_response(200, @api.index)
      end
    end

    def handle_show_request(request)
      check_authorization(request) do |user|
        setting = @api.show(request.params["key"])
        if setting
    ...
    def handle_update_request(request)
      check_authorization(request, write_required: true) do |user|
        result = @api.update(post_params(request)["settings"], changed_by(user))
        if result[:success]
    ...
    def handle_history_request(request)
      check_authorization(request) do |user|
        history = @api.history(request.params["key"], limit: request.params["limit"], offset: request.params["offset"])
        if history
    ...
    def handle_last_updated_at_request(request)
      check_authorization(request) do |user|
        json_response(200, @api.last_updated_at)
      end
    end

    def handle_updated_since_request(request)
      check_authorization(request) do |user|
        json_response(200, @api.updated_since(request.params["time"]))
      end
    end
